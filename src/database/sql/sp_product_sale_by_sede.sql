...
BEGIN
    -- 1. IDENTIFICACIÓN DE CAJAS (Lógica de Fecha de Negocio: 6:00 AM)
    ...
    WHERE l.local_number = p_id_local 
      ...
      AND (
        CASE
            ...
            WHEN ws.shift_name::text <> 'MAÑANA'::text AND EXTRACT(hour FROM (cr.opennig_date AT TIME ZONE 'America/Lima')) < 6 THEN 
    ...
    -- PASO 1: Sumar montos por método y turno (Sin anidar funciones)
    pre_pagos AS (
        SELECT 
            COALESCE(bv.nombre_turno, 'TOTAL GENERAL') as grupo,
    ...
    -- PASO 2: Construir el JSON de pagos con el monto ya sumado
