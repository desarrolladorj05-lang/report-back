...
export class CashPettyReportService {
  ...
  async getCashPettyReport(
    ...
    this.logger.debug(
      `getCashPettyReport llamado con periodo: ${year}-${month}`,
    );

    const result = await this.reportRepository.getCashPettyReport(year, month);
  ...
  async getCashPettyDetail(id: string): Promise<CashPettyDetailResult> {
    ...
    if (!result) {
      ...
      throw new BadRequestException(
        "No se encontró la caja chica especificada",
      );
    }
